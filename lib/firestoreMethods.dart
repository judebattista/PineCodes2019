...
class FirebaseFirestoreService {
  ...
  Future<Users> createUser(
    ...
    final TransactionHandler createTransaction = (Transaction tx) async { //creating firestore transaction
      DocumentSnapshot ds;
      if (userId != "") { //Default itemID is "" ????
        ds = await tx.get(usersCollection.document(userId));
      } else {
        ds = await tx.get(
            usersCollection.document(userId)); //Note that the Equipment class is named 'Items' in collections
    ...
    if (userId != "") {
      return usersCollection.document(userId).get().then((doc) {
        if (doc.exists) {
          print("DNE");
